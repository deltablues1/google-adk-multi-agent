...
class VendorInvoiceService(BaseERPService):
    ...
    async def create_from_ubl(
        ...
        # ── Buyer OIB resolution ──
        # Primary: company_settings store (CompanyService, multi-tenant aware).
        # Fallback: static company_config (single-company deployments).
        # Empty OIB skips the buyer-identity check in the parser; mismatch is a hard error.
        buyer_oib = ""
        ...
        if not buyer_oib:
            try:
                ...
                buyer_oib = get_company_config().oib or ""
                if buyer_oib:
                    buyer_oib_source = "company_config"
            except Exception:
                pass
    ...
    async def get_inbound_compliance_report(
        ...
        Always returns the report dict; CSV conversion is available via
        compliance_report_to_csv().
        """
        import calendar
        from datetime import date as date_type
    ...
    @staticmethod
    def compliance_report_to_csv(report: dict) -> "tuple[str, str]":
        """Convert compliance report dict to CSV. Returns (csv_text, filename) —
        HTTP response wrapping is the route's job, not the service's."""
        import io, csv
