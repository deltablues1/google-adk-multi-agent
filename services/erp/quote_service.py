...
class QuoteService(BaseERPService):
    ...
    async def create_outbound_b2g_from_quote(
        ...
        # Same race-free idempotency as the B2B path: deterministic doc ID +
        # create() precondition. IDs live in separate collections (invoices_b2g),
        # so reusing the q2i- prefix is safe.
        deterministic_id = f"q2i-{quote_id}"
        try:
            invoice = await get_outbound_b2g_service().create(
                payload, ctx, invoice_id=deterministic_id, fail_if_exists=True
            )
        except AlreadyExists:
            logger.info(
                f"[QuoteService] Concurrent B2G create for quote {quote_id} — "
                f"returning existing invoice {deterministic_id}."
            )
            existing = await get_outbound_b2g_service().get(deterministic_id, ctx)
            existing["_already_exists"] = True
            return existing
