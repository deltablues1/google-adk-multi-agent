...
# ---------------------------------------------------------------------------
# Marketing: honest not_implemented
# ---------------------------------------------------------------------------

class TestMarketingHonesty:
    @pytest.mark.asyncio
    async def test_youtube_upload_not_implemented(self):
        from tools.adk_tools.marketing_adk_tools import upload_to_youtube

        result = await upload_to_youtube("gs://bucket/x.mp4", "Test")
        assert result["status"] == "not_implemented"
        assert "video_id" not in result
        assert "mock" not in str(result).lower()

    @pytest.mark.asyncio
    async def test_ads_draft_not_implemented(self):
        from tools.adk_tools.marketing_adk_tools import create_google_ad_draft

        result = await create_google_ad_draft(
            "Camp", "Headline", "Description", "https://example.com"
        )
        assert result["status"] == "not_implemented"
