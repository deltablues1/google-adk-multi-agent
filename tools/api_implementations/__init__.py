...
try:
    from .google_ads import register_google_ads_tools
    GOOGLE_ADS_AVAILABLE = True
except ImportError as e:
    logger.warning(f"Google Ads implementation not available: {e}")
...
def register_all_tools(tool_registry) -> Dict[str, bool]:
    ...
    if GOOGLE_ADS_AVAILABLE:
        try:
            register_google_ads_tools(tool_registry)
            status['google_ads'] = True
            logger.info("Google Ads tools registered")
        except Exception as e:
            logger.error(f"Failed to register Google Ads tools: {e}")
